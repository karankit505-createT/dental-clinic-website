    ...
    function openReportsModalForItem(item, doctorNameDisplay, genderDisplay) {
        ...
        if (reports.length > 0) {
            reports.forEach((report, idx) => {
                let dateDisplay = (typeof formatReportDate === 'function') 
                    ? formatReportDate(report.report_date || report.upload_date) 
                    : (report.report_date || report.upload_date || 'Date not recorded');
                ...
                buttonsList.push(`
                    <button type="button" onclick="if(typeof viewPatientDocument==='function'){viewPatientDocument('${escapeHtml(report.url)}', '${escapeHtml(item.patient_name)}_${escapeHtml(report.name)}')}else{window.open('${escapeHtml(report.url)}', '_blank')}" style="width: 100%; padding: 14px 18px; background: linear-gradient(135deg, #0f766e 0%, #0d9488 100%); color: white; border: none; border-radius: var(--radius-sm); font-weight: 700; font-size: 0.92rem; cursor: pointer; text-align: left; display: flex; align-items: center; justify-content: space-between; transition: all 0.2s; box-shadow: 0 2px 6px rgba(15, 118, 110, 0.25);">
                        <div style="display: flex; flex-direction: column; gap: 4px;">
                            ...
                            <span style="font-size: 0.8rem; color: rgba(255, 255, 255, 0.82); font-weight: 500; display: flex; align-items: center; gap: 4px; margin-left: 24px;">
                                📅 ${escapeHtml(dateDisplay)}
                            </span>
                        </div>
                        <span style="font-size: 0.76rem; background: rgba(255,255,255,0.22); padding: 6px 14px; border-radius: 20px; font-weight: 600; white-space: nowrap; flex-shrink: 0; margin-left: 12px;">Open / Download ↗</span>
                    </button>
                `);
            });
        }
